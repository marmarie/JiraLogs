...
public class TestHttp {
    ...
    public static JiraIssue getIssueListForLog(String basicIssueId,ArrayList<JiraIssue> alreadyLoggedList){
        JiraIssue needLogJira = new JiraIssue();
        needLogJira.setId(basicIssueId);
        for(JiraIssue jiraIssue:alreadyLoggedList){
            LinkedHashMap<String, String> logs = jiraIssue.getWorkLogs();
            LinkedHashMap<String, String> needToLogMap = new LinkedHashMap<>();
            for (String id:logs.keySet()){
                if(!logs.get(id).equals("28800")){
                    String leftToLog = String.valueOf(28800L - Long.parseLong(logs.get(id)));
    ...
    private static String transformData(){
        //todo
        return "";
    }
